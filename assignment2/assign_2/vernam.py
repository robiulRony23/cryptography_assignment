			...
			for i3 in range(0,len(rrre[i+2])):
				for i4 in range(0,len(rrre[i+3])):
					for i5 in range(0,len(rrre[i+4])):
						pcopy=pre.copy();
						k=[rrre[i][i1],rrre[i+1][i2],rrre[i+2][i3],rrre[i+3][i4],rrre[i+4][i5]]#Prev should not changed
						m5=decode5(i,k);
						# if(i==0):
						# 	print(m5)
						for j in range(0,10):
							...
							for jj in range(0,len(prespls[j])):
								...
								if(jj!=len(prespls[j])-1):
									...
									if(len(prefixSearch(trie,prespls[j][jj]))==0):
										...
										cccc+=1
										if(m5[4]==' ' or m5[4]=='.' or m5[4]==',' or m5[4]=='-' or m5[4]=='?' or m5[4]=='(' or m5[4]==')' or m5[4]=='!'):
